...
//data chunk found, read samples now
  ...
  while (!(sampleBuffer = malloc(sampleBufferLen * sizeof(char))))
  {
    //posibly add error handling if it takes long for malloc to succeed
  }
  fread((void*)sampleBuffer, sampleBufferLen, 1, fp);
  fclose(fp);
switch (header.numBitsPerSample)
{
  case SAMPLE_8BIT:
...
wav_sound* convertFrom32Bit(char* buffer, size_t length)
{
  ...
  {
    uint32_t sample = 0;
    // buffer[n*SAMPLE_SIZE_32_BIT] contains high fidelity bits, this converts to 24bit pcm.
